...
def dessine_carte(taille : int, carte : tuple, tortue : turtle.Turtle):
    ...
    tortue.write(carte[0], font=("", int(taille/5))) # Le Turtle écrit la face de la carte
...
def evalue_partie(main_joueur : list, main_croupier : list):
    ...
    valeur_croupier = blackjack.compte_valeur(main_croupier) # On stocke la valeur de la main du croupier dans la variable valeur_croupier
    valeur_joueur = blackjack.compte_valeur(main_joueur) # On stocke la valeur de la main du joueur dans la variable valeur_joueur
...
dessineur.write("Balance:", font=('', 40), align="center")
...
annonce_balance.write(balance, font=('', 40), align="center") # Écrit la balance de l'utilisateur
...
dessineur.write("Record Actuel: ", font=('', 20, "bold"), align="center") # On écrit "Record Actuel" dans le panneau
    ...
    while not ok: # Tant que l'utilisateur n'a pas entrer un pari valide
        try:
            ...
            if pari < 0: # Si le pari est un nombre négatif
                ...
                annonce_balance.write(balance, font=('', 40), align="center") # On remet la balance mise à jour
...
def jouer(action : str):
    ...
    if action == "hit": # Si le joueur a décidé de Hit
        ...
        if blackjack.compte_valeur(main_joueur) > 21: # Si la valeur de la main du joueur a dépassée 21
            ...
            annonceur_gagnant.write("Perdu!", font=('', 80, 'bold'), align="center") # On écrit "Perdu!" sur l'écran
            ...
            if balance == 0: # Si le joueur n'a plus d'argent
                ...
                with open("record.txt", "r") as record: # On ouvre le fichier record.txt
                    if max_balance > int(record.read()): # On regarde si le joueur a atteint un nouveau record
                        ...
                        annonceur_gagnant.write("Félicitations! Vous avez battu le record avec votre balance maximale de " + str(max_balance) + ".", font=('', 35, 'bold'), align="center") # On écrit un message de félicitation, et on affiche la balance maximale
                    else:
                        annonceur_gagnant.color("red") # Change la couleur de l'encre du Turtle annonceur_gagnant au vert
                        annonceur_gagnant.write("Vous avez perdu le jeu. Votre balance maximale était de " + str(max_balance) + ".", font=('', 35, 'bold'), align="center") # On dit au joueur qu'il a perdu, et on lui dit sa balance maximale atteinte
        ...
        resultat = evalue_partie(main_joueur, main_croupier) # Avec la fonction evalue_partie, on détermine le gagnant
        if resultat == True: # Si le joueur a gagné
            annonceur_gagnant.color(57, 255, 20) # Change la couleur de l'encre du Turtle annonceur_gagnant au vert
            annonceur_gagnant.write("Gagné!", font=('', 80, 'bold'), align="center") # On écrit "Gagné!"
            ...
            # On met à jour la balance de l'utilisateur
            annonce_balance.clear()
            annonce_balance.write(balance, font=('', 40), align="center")
            ...
            annonceur_gagnant.write("Perdu!", font=('', 80, 'bold'), align="center") # On écrit perdu
            if balance == 0: # Si le joueur n'a plus d'argent
                time.sleep(2) # On attend 2 secondes
            ...
            annonceur_gagnant.write("Égalité.", font=('', 80, 'bold'), align="center") # On écrit "Égalité"
